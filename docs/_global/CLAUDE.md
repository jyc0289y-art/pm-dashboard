...
### 위반 시
- 보고가 채팅에 흩어져 진행·타임라인 파악 비용↑. **과거 보고 누락 시 프로젝트 전체 맥락 유실**. 타임스탬프·기록물·복구점검 누락 시 추적성·복원력 약화. **캡쳐가 1열 나열·설명 없음이면 사용자 이해 비용↑ (규칙 7 위반)**. **요약 결의에 원문 딥링크가 없으면 오너가 세부를 찾을 수 없어 회신 자체가 불가 — 보고가 '읽었다' 수준에서 멈춤 (규칙 8 위반)**. **분과 보유 프로젝트가 divisions 탭·최신화를 누락하면 오너의 분과 소통 창구 상실 (규칙 9 위반)**.
