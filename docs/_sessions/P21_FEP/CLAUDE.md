...
> **AI(Claude)는 오디오를 들을 수 없고 영상 재생을 관찰할 수 없다. 이 구조적 한계를 매 순간 인식하며 작업하라.**

## ⛔ 절대 금지 (반복된 실수 패턴)

### 1. "정량 지표 100%" 자만 금지 (TRAP-040)
- silencedetect `nearest timing` 매칭은 **내레이터와 대사가 혼재된 오디오에서 완전히 실패**한다
- 과거 사례: "...응." 말풍선 실제 매칭 시점 **14초 오차** (내레이터 "민준이의 머릿속..."에 잘못 snap)
- "21/21 (100%) 정합"이라 보고했지만 실제 사용자 체감은 "전혀 안맞음"
- ✅ **원칙**: 타이밍 매칭은 반드시 **whisper word-level + 내용(text) 매칭**으로만 수행. silencedetect 단독 사용 금지.

### 2. "코드에 반영 = 시각 구현" 착각 금지 (TRAP-039 반복)
- 벤치마킹 규칙을 코드에 구현했다고 실제 프레임에서 그 규칙이 시각적으로 지각된다는 보장 없음
- 과거 사례: 꼬리→입 방향 로직은 코드에 있었지만 에셋 꼬리 + 추가 꼬리가 중첩되어 "드래그앤드롭 모양새"
- ✅ **원칙**: 모든 "개선"은 **사용자 시각 검증 통과 후**에만 "개선됨" 표기. 내 메트릭이 통과해도 사용자가 "안 됐다"면 메트릭이 틀린 것.

### 3. "벤치마킹 학습했다"면서 실제 영상 측정 없이 추정 금지
- 텍스트 기반 벤치마킹(블로그/설명)은 **구현 방향 오차 500ms 이상 발생 가능**
- 과거 사례: "자막은 대사 시작 직후" ← 측정 없이 "대사 시작 0.4초 전"으로 정반대 구현 (반대 방향 900ms)
- ✅ **원칙**: 신규 규칙 도입 시 반드시 `reverse_engineering/` 파이프라인(yt-dlp + faster-whisper + ffmpeg scdet)으로 **실측값 확보 후 적용**

### 4. 사용자 "안 맞아" = 즉시 접근 재검토
- 사용자가 체감 문제를 지적하면 **메트릭이 불완전**하다는 뜻
- 같은 방법으로 "이번에는 진짜"라고 주장하지 말고 **접근 자체를 바꿀 것**
- ✅ **원칙**: 같은 이슈가 2회 이상 재발하면 **측정 도구/기준 자체를 교체**

## 📏 측정 기반 FEP 표준값 (실측 확보)

레퍼런스 3편 측정 결과 (`reverse_engineering/reports/comprehensive_report.md`):
- **씬 duration**: 평균 4.9s (Psych2Go 7s / 짤툰 3.5-4s) — **FEP 현재 20~40s → 대폭 축소 필요**
- **대사 세그먼트 duration**: 중간 1.1~2.0s (Psych2Go 2.1s / 짤툰 1.2s)
- **발화 간 pause**: 중간 0~60ms (거의 연속) — **FEP TTS 과도한 무음 삽입 금지**
- **말풍선당 문자 수**: 중간 9~15자, 최대 40자 (**FEP 상한 14자 권장**)
- **타이밍 매칭 방식**: 반드시 whisper word-level + SequenceMatcher 내용 매칭

## 🔧 영상 개선 시 필수 절차

1. **측정 우선**: 추정 대신 `reverse_engineering/scripts/` 파이프라인 실행
2. **word-level sync**: FEP TTS도 whisper로 전사 → 매니페스트 텍스트와 내용 매칭 후 snap
3. **side-by-side 비교**: 개선 전/후 same-timestamp 프레임 나란히 비교 필수
4. **사용자 검수 이전 자체 검증**: Layer 1~5 자체 QA 체크리스트 통과 + 회귀 비교 보고서
5. **"개선됨" 선언 유예**: 사용자 시각 검증 결과 확인 전에는 "개선 시도" 로만 표기
...
- ✅ Type E(구조라벨): 초반 1.5초 센터 전환 카드(큰 폰트+배경) → 좌상단 작은 라벨 잔류 (짤툰 벤치마킹)
- ✅ **A/V 정합성 자동 검증 필수** (TRAP-038): manifest의 A/B 타이밍은 반드시 `silencedetect` 또는 VTT 기반으로 auto-snap하여 실제 발화 시점 ±200ms 내 정합 유지. 수동 추정 금지.
- ✅ **말풍선 꼬리→입 커스텀 연결 필수** (TRAP-039): 에셋의 베이크드-인 꼬리는 머리 위에서 끝나므로 Pillow로 꼬리 바닥→입까지 커스텀 삼각형/점선 꼬리 추가 렌더링. "꼬리가 입을 향한다" 벤치마킹 규칙을 시각적으로 구현.
- ❌ **벤치마킹 규칙의 코드 반영 ≠ 시각 구현** (TRAP-039 교훈): 규칙을 코딩했다고 실제 프레임에서 그 규칙이 지각된다는 보장 없음 → 반드시 렌더링 결과를 사용자 시각으로 검증
...
- ✅ 말풍선 위치는 PORTRAIT_RECIPES의 head_y 기반 동적 배치 (캐릭터 머리 위 negative space, HAIR_OVERLAP=35px 허용, 얼굴 보호)
- ✅ 말풍선 꼬리 방향은 speaker_x_ratio 기반 자동 + 꼬리는 입/얼굴 방향 지시 (mouth_y = head_y + target_h * 0.18)
- ✅ 말풍선 크기는 font_size 비례 패딩 (pad_w=font_size×1.3, pad_h=font_size×1.1, tail=font_size×0.6) — 균일 크기 금지 (TRAP-036)
- ✅ 다인 씬(2인+) 화자 인지: SPEAKER_BUBBLE_TINT로 캐릭터별 풍선 배경색 틴팅 (수빈=핑크, 민준=블루, 지우=그린)
- ❌ 말풍선 Y 좌표 하드코딩 금지 — 반드시 PORTRAIT_RECIPES의 height_ratio + crop_offset 기반 head_y 동적 계산 (TRAP-035)
...
### 13. 채널 벤치마킹 인사이트 (포트레이트 시네마틱 + 주석/자막)

> **벤치마킹 대상**: Psych2Go (1300万), 짤툰 (300万), Kurzgesagt (2300万), 영상툰 장르, 나무위키/Blambot/CLIP STUDIO 참고

#### A. 포트레이트 시네마틱 연출 전략 (성공 채널 공통)
| 기법 | 설명 | FEP 적용 |
|------|------|----------|
| **표정 변화 기반 감정 전달** | 같은 캐릭터의 표정 변형 3~5종으로 감정 아크 표현 (Psych2Go) | LP 모션 + 표정 포즈 라이브러리 |
| **배경 색감 전환** | 감정에 따라 동일 장소 BG의 색온도/채도 변화 (짤툰) | location variants + Pillow 필터 |
| **화면 분할/시점 전환** | 2인 대화 시 좌우 분할, 시점 변경 시 컷 전환 (영상툰) | PORTRAIT_RECIPES 포지션 활용 |
| **정적 타이밍** | 감정 전환점에 0.5~2초 정적 + BGM 전환 (공통) | 비트 클립 + BGM 설계 |
| **일러스트 퀄리티 일관성** | 에피소드 전체에서 스타일 통일 (Kurzgesagt) | 듀얼 스타일 A/B + 캐릭터 시트 |

#### B. 텍스트 오버레이 타입별 프로 연출 규칙

| 타입 | 기존 | 개선 (벤치마킹) | 참고 채널 |
|------|------|----------------|----------|
| **Type A (말풍선)** | font_size 비례 패딩 | 유지 + 다이아몬드 텍스트 배치 | 만화 업계 |
| **Type B (생각풍선)** | 연한 블루-그레이 텍스트 | 연한 블루 #B0D4F1 (내면독백 시각 구분) | 영상툰 |
| **Type C (심리학 용어)** | 단순 하단 바 | 노란 밑줄 강조 + 한국어 큰 폰트 + 영어 작은 폰트 2줄 | Psych2Go |
| **Type D (킬링라인)** | 반투명 백드롭 | 전체화면 40% 디밍 + 56~72px + 따옴표「」 + 화자명 | 영상툰/짤툰 |
| **Type E (구조라벨)** | 좌상단 작은 패널 | 초반 센터 전환 카드(큰 폰트) → 좌상단 잔류 | 짤툰 |

#### C. 텍스트 오버레이 타이포그래피 표준

| 타입 | 폰트 크기 | 색상 | 위치 | 효과 |
|------|----------|------|------|------|
| A (말풍선) | 44px Bold | #282828 | 캐릭터 head_y 위 | font_size 비례 패딩 |
| B (생각풍선) | 40px Regular | #B0D4F1 (연한 블루) | 캐릭터 head_y 위 | 점선 풍선 + 연한 색 |
| C (심리학 용어) | 36px Bold + 24px | #FFFFFF + 노란 밑줄 | 하단 바 (120px) | 2줄: 한국어 + (영어) |
| D (킬링라인) | 56~72px Bold | #FFFFFF | 센터 | 전체 디밍 + 따옴표 + 화자명 |
| E (구조라벨) | 36px → 24px | #FFFFFF | 센터 → 좌상단 | 전환 카드 → 잔류 라벨 |

#### D. 다인 씬 화자 인지 전략 (만화 + 영상 혼합)
1. **말풍선 꼬리** → 화자 입/얼굴 방향 (1차 단서)
2. **색상 틴팅** → SPEAKER_BUBBLE_TINT (2차 단서, 다인 씬 전용)
3. **LP 모션 활성화** → 현재 화자만 talking 모션 (3차 단서)
4. **위치 일관성** → 같은 씬 내 캐릭터 좌우 위치 고정

### 14. A/V 정합성 자동 검증 시스템 (TRAP-038 대응)

> **원칙**: AI는 오디오를 직접 들을 수 없음. manifest의 말풍선 타이밍이 실제 TTS 발화와 일치하는지 객관적 신호로 자동 검증해야 함.

#### A. silencedetect 기반 대사 구간 추출
```bash
ffmpeg -i {audio}.mp3 -af "silencedetect=noise=-30dB:d=0.3" -f null - 2>&1 \
  | grep -E "silence_(start|end)"
```
- 각 오디오 파일을 분석하여 정적 구간(0.3s+)을 추출
- 정적의 역(complement)이 발화 구간 `[start, end]` 리스트

#### B. 말풍선 타이밍 auto-snap
- 각 A/B 타입 말풍선의 `start_ratio × audio_dur`에서 가장 가까운 발화 시작 탐색
- `new_start_ratio = (speech_start - BUBBLE_TIMING_DELAY_SEC) / audio_dur`
- `BUBBLE_TIMING_DELAY_SEC = 0.4` (build_video_v3.py에 이미 존재)
- 결과: 말풍선이 실제 발화 시작 ±200ms 이내에 표시됨
...
| ≤ 200ms | ✅ 정합 | 통과 |
| 200~500ms | 🟡 경미 | 시각 확인 |
| > 500ms | 🔴 어긋남 | auto-snap 필수 |

#### D. TTS 생성 단계 예방
- edge-tts 실행 시 `--write-subtitles {name}.vtt` 옵션 필수
- VTT 기반 word-level timing으로 manifest 자동 생성 가능 (silencedetect보다 정확)
- EP.02부터 TTS 생성 워크플로우에 VTT 필수화

#### E. 실전 효과 (EP.01 적용)
- **Before**: 21개 A/B 말풍선 중 19개(90.5%) 타이밍 불일치, 최대 ±1548ms
- **After (auto-snap)**: 21/21 (100%) 정합, 최대 편차 200ms 이내
- 사용자가 체감한 "정합성 품질 문제" 근본 해결
